...
export function extractStravaMetrics(
    ...
    // Filter to running activities in last 4 weeks
    const recentRuns = activities.filter(a => {
        const actDate = a.date instanceof Date ? a.date : new Date(a.date);
        const isRecent = actDate >= fourWeeksAgo && actDate <= now;
        const isRun = !a.type || a.type === 'Run';
        return isRecent && isRun;
    });

    // Calculate 4-week average
    const totalDistance = recentRuns.reduce((sum, a) => sum + a.distance_km, 0);
    const avgWeeklyDistance = totalDistance / 4;

    // Find last major event (>40km)
    ...
    const lastMajorEvent = sortedActivities.find(a => a.distance_km >= 40);
    let daysSinceEvent: number | undefined;

    if (lastMajorEvent) {
        const eventDate = lastMajorEvent.date instanceof Date ? lastMajorEvent.date : new Date(lastMajorEvent.date);
        daysSinceEvent = Math.floor((now.getTime() - eventDate.getTime()) / (24 * 60 * 60 * 1000));
    }
    ...
    const thisWeekDistance = thisWeekRuns.reduce((sum, a) => sum + a.distance_km, 0);

    // Calculate ACWR
    const acwr = avgWeeklyDistance > 0 ? thisWeekDistance / avgWeeklyDistance : undefined;
    ...
    return {
        ...
        acwr: acwr ? Math.round(acwr * 100) / 100 : undefined,
        lastEvent: lastMajorEvent ? {
            name: lastMajorEvent.name,
            distance_km: lastMajorEvent.distance_km,
            date: lastMajorEvent.date instanceof Date ? lastMajorEvent.date : new Date(lastMajorEvent.date),
        } : undefined,
